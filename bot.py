...
async def finish(cb: CallbackQuery):
    ...
    await cb.message.answer("Отлично. Ждем тебя!")
    ...
    if u["stage"] == "form":
        ...
        if u["step"] >= len(FORM_STEPS):
            ...
            await message.answer(
                ...
                "После просмотра ответьте на вопрос:\n"
...
async def scrum_answer(cb: CallbackQuery):
    ...
    await cb.message.answer(
        "📺 Видео про фасилитацию:\n"
        "Перейдите по ссылке и ознакомьтесь с материалом."
...
async def fac_answer(cb: CallbackQuery):
    ...
    await cb.message.answer(
        ...
        "Приглашаем вас на онлайн-встречу с основателем школы и менеджером школы:\n"
        "🗓 Среда, 12:30\n"
        "📍 Ссылка в Zoom\n"
    ...
    while True:
        ...
        for uid, u in list(users.items()):
            ...
            if u["state"] == "interview":
                ...
                if interview_at - now <= timedelta(days=1) and not u["rem_1d_interview"]:
                    await bot.send_message(
                        uid,
                        "📅 Напоминание!\n"
                ...
                if interview_at - now <= timedelta(hours=1) and not u["rem_1h_interview"]:
                    await bot.send_message(
                        uid,
